...
The inner function must close over factor — do not use a global.`,
    ...
    hints: ["def make_multiplier(factor): def inner(x): return x * factor","return inner at the end of make_multiplier."],
...
Each call to the returned function increments an internal count starting at start and returns the new count.`,
    ...
    hints: ["Use a list or nonlocal to mutate state across calls.","count = [start] then count[0] += 1 works without nonlocal."],
    ...
    pass
`,
    solutionCode: `def make_counter(start=0):
    count = [start]
...
Use yield — do not build and return a list.`,
    ...
    hints: ["Track previous two values in a loop.","yield the next sum, then shift the window forward."],
    approach: `Generator functions pause with yield. Track two running values and emit their sum each step.`,
    starterCode: `def fibonacci_gen(count):
    # Write your code here
    pass
`,
    solutionCode: `def fibonacci_gen(count):
    a, b = 0, 1
...
Use trial division — no imports required.`,
    ...
    hints: ["For each candidate n, test divisibility by 2 .. int(n**0.5).","Skip even numbers after yielding 2."],
    approach: `Test each odd candidate for divisors up to its square root. Yield when none divide evenly.`,
    starterCode: `def primes_up_to(limit):
    # Write your code here
    pass
`,
    solutionCode: `def primes_up_to(limit):
    if limit < 2:
        return
    yield 2
    n = 3
...
Use a dict comprehension with text.count(word) or an equivalent comprehension-based approach.`,
    ...
    hints: ["words = text.lower().split()","{word: words.count(word) for word in set(words)} removes duplicates as keys."],
...
`,
    tests: [
      { label: `Simple sentence`, call: `word_frequency("The cat and the dog")`, expected: `{"the": 2, "cat": 1, "and": 1, "dog": 1}` },
      { label: `Repeated words`, call: `word_frequency("Go go GO")`, expected: `{"go": 3}` }
    ],
  }),
...
Define class Circle(Shape) with __init__(self, radius) and area() returning pi * radius ** 2 (use 3.14159 for pi).`,
    ...
    hints: ["class Circle(Shape):","Override area() in Circle; call super().__init__() if you add one."],
...
- fahrenheit getter returns _celsius * 9/5 + 32; setter converts from F to C`,
    ...
    hints: ["@property def celsius(self): return self._celsius","@celsius.setter converts incoming values to _celsius."],
...
Override pay() to return base_salary + bonus using super().pay().`,
    ...
    hints: ["super().__init__(name, base_salary) in Manager.__init__","return super().pay() + self.bonus"],
...
On __exit__, append the string "done" to target_list (always, even if an exception occurred).`,
    ...
    hints: ["__enter__ returns self.target_list","__exit__ receives exc_type, exc_val, exc_tb — append then return False."],
    approach: `Context managers run setup/teardown around a with block. __exit__ runs even when errors occur.`,
    starterCode: `class AppendList:
...
Repeated calls with the same args return the cached value without re-running the function.`,
    ...
    hints: ["cache = {} inside memoize, before defining wrapper.","if args not in cache: cache[args] = func(*args)"],
...
Return the original function's result unchanged.`,
    ...
    hints: ["CALL_LOG = [] at module level in the student's code area.","CALL_LOG.append(func.__name__) inside wrapper before calling func."],
...
If any argument is <= 0, raise ValueError("Arguments must be positive") before calling the wrapped function.`,
    ...
    hints: ["for arg in args: if arg <= 0: raise ValueError(...)","return func(*args, **kwargs) when all pass."],
    approach: `Validate inputs in the wrapper before delegating. Decorators are ideal for cross-cutting checks.`,
    starterCode: `def require_positive(func):
    # Write your code here
    pass
`,
    solutionCode: `def require_positive(func):
    def wrapper(*args, **kwargs):
        for arg in args:
            if arg <= 0:
...
Return a list of valid IPv4 strings (four dot-separated digit groups). Each octet is 0–255.`,
    ...
    hints: ["Find \\\\d{1,3} groups separated by dots.","Filter matches where all four parts int(part) <= 255."],
...
`,
    tests: [
      { label: `Two addresses`, call: `extract_ipv4("Host 192.168.0.1 and 10.0.0.5 seen")`, expected: `["192.168.0.1", "10.0.0.5"]` },
      { label: `Rejects invalid`, call: `extract_ipv4("Bad 999.999.999.999 ok 127.0.0.1")`, expected: `["127.0.0.1"]` }
    ],
  }),
...
Return a dict with keys level, user, id, msg using re.search and named groups.`,
    examples: [
      { input: `parse_log_line("[INFO] user=bob id=1 msg=OK")`, output: `{"level": "INFO", "user": "bob", "id": 1, "msg": "OK"}` }
    ],
    constraints: ["Use named groups (?P<name>...)","Return None if the line does not match"],
    hints: ["Pattern: \\\\[(?P<level>\\\\w+)\\\\] user=(?P<user>\\\\w+) id=(?P<id>\\\\d+) msg=(?P<msg>.+)","return match.groupdict() when match else None"],
...
`,
    tests: [
      { label: `Valid line`, call: `parse_log_line("[ERROR] user=alice id=42 msg=File not found")`, expected: `{"level": "ERROR", "user": "alice", "id": 42, "msg": "File not found"}` },
  ...
  buildBasicsProblem({
    ...
    hints: ["merged = {**json.loads(a), **json.loads(b)}","return json.dumps(merged, sort_keys=True)"],
...
`,
    tests: [
      { label: `Merge two objects`, call: `merge_json('{"a": 1, "b": 2}', '{"b": 9, "c": 3}')`, expected: `"{\\"a\\": 1, \\"b\\": 9, \\"c\\": 3}"` },
      { label: `Override nested key`, call: `merge_json('{"x": 1}', '{"y": 2}')`, expected: `"{\\"x\\": 1, \\"y\\": 2}"` }
    ],
  }),
...
Write validate_score(score) that raises InvalidScoreError("Score must be 0-100") when score is outside 0..100, otherwise returns score.`,
    ...
    hints: ["class InvalidScoreError(Exception): pass","raise InvalidScoreError(\"Score must be 0-100\")"],
    approach: `Custom exceptions communicate domain errors. Validate range before returning the value.`,
    starterCode: `class InvalidScoreError(Exception):
    pass
...
def validate_score(score):
    ...
    pass
`,
    solutionCode: `class InvalidScoreError(Exception):
    pass
...
Return None (do not raise) when b is zero or either argument is not int/float.`,
    ...
    hints: ["if not isinstance(a, (int, float)): return None","if b == 0: return None before dividing."],
...
If the file has fewer than n lines, return all lines.`,
    ...
    hints: ["lines = open(path).read().splitlines()","return lines[-n:] when n > 0"],
    approach: `splitlines() removes newlines. Slice from the end to get the last n entries.`,
    starterCode: `def read_last_lines(path, n):
    # Write your code here
    pass
`,
    solutionCode: `def read_last_lines(path, n):
...
Write one JSON object per line (JSON Lines / .jsonl format) using json.dumps per record and a trailing newline.`,
    ...
    hints: ["with open(path, 'w') as f:","f.write(json.dumps(record) + '\\n') for each record"],
...
Return None if the string is not exactly three dot-separated integer parts.`,
    ...
    hints: ["parts = version.split('.')","Check len(parts) == 3 and each part is digit-only."],
    approach: `Split on dots, validate three numeric segments, convert to ints — a lightweight version parsing utility.`,
    starterCode: `def parse_version(version):
    # Write your code here
    pass
`,
    solutionCode: `def parse_version(version):
    parts = version.split(".")
    if len(parts) != 3:
        return None
    nums = []
...
Example: input "1 2 3 4" prints 6.`,
    ...
    hints: ["nums = map(int, input().split())","sum(n for n in nums if n % 2 == 0)"],
    approach: `Parse stdin, filter evens with a generator expression, aggregate with sum().`,
    starterCode: `# Read integers from stdin and print the sum of evens
`,
    solutionCode: `nums = map(int, input().split())
print(sum(n for n in nums if n % 2 == 0))
`,
    tests: [
      { label: `Mixed numbers`, expectedStdout: `12`, stdin: `1 2 3 4 5 6` },
      { label: `All odd`, expectedStdout: `0`, stdin: `1 3 5` },
      { label: `Single even`, expectedStdout: `8`, stdin: `8` }
    ],
  }),
...
Use the re module.`,
    ...
    hints: ["re.sub(r\"\\\\b\\\\w+@\\\\w+\\\\.\\\\w+\\\\b\", \"[REDACTED]\", text)","text = input()"],
...
`,
    tests: [
      { label: `Two emails`, expectedStdout: `Contact [REDACTED] or [REDACTED] today`, stdin: `Contact alice@test.com or bob@site.org today` },
      { label: `No emails`, expectedStdout: `Hello world`, stdin: `Hello world` }
    ],
  }),
...
Parse with json.loads and print: Hello, {name}! Score: {score}`,
    ...
    hints: ["data = json.loads(input())","print(f\"Hello, {data['name']}! Score: {data['score']}\")"],
...
`,
    tests: [
      { label: `Basic greet`, expectedStdout: `Hello, Ana! Score: 95`, stdin: `{"name": "Ana", "score": 95}` },
      { label: `Another user`, expectedStdout: `Hello, Bob! Score: 72`, stdin: `{"name": "Bob", "score": 72}` }
    ],
  }),
];
