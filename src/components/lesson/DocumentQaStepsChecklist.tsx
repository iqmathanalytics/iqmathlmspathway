...
const STEPS = [
  { id: 1, title: "Install Libraries", snippet: "!pip install sentence-transformers faiss-cpu ...", color: "emerald" },
  { id: 2, title: "Import Libraries", snippet: "from sentence_transformers import ...", color: "teal" },
  { id: 3, title: "Define Knowledge Base", snippet: "documents = [...]", color: "blue" },
  { id: 4, title: "Encode Embeddings", snippet: "embedding_model.encode(documents)", color: "indigo" },
  { id: 5, title: "Build FAISS Index", snippet: "index = faiss.IndexFlatIP(dimension)", color: "violet" },
  { id: 6, title: "Load Generator", snippet: "generator = pipeline('text2text-generation' ...)", color: "purple" },
  { id: 7, title: "Create RAG Function", snippet: "def rag_answer(query): ...", color: "fuchsia" },
  { id: 8, title: "Run Chat Loop", snippet: "run_qa_bot()", color: "pink" },
] as const;
...
const DONE_COLORS = {
  ...
  indigo:  { border: "border-indigo-200",  bg: "bg-indigo-50",  title: "text-indigo-800",  check: "text-indigo-500"  },
  ...
  fuchsia: { border: "border-fuchsia-200", bg: "bg-fuchsia-50", title: "text-fuchsia-800", check: "text-fuchsia-500" },
  pink:    { border: "border-pink-200",    bg: "bg-pink-50",    title: "text-pink-800",    check: "text-pink-500"    },
} as const;

export function DocumentQaStepsChecklist() {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      {/* ── Header ── */}
      <div className="border-b border-slate-200 bg-gradient-to-r from-teal-50 to-emerald-50 px-4 py-4">
        ...
        <div className="mt-3 space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-[11px] font-bold ${allDone ? "text-teal-700" : "text-slate-500"}`}>
              {pct}%
            </span>
          </div>
          <div className="h-2 w-full overflow-hidden rounded-full bg-slate-100">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                allDone ? "bg-teal-500" : "bg-teal-400"
              }`}
              style={{ width: `${pct}%` }}
            />
          </div>
        ...
        {allDone && (
          <div className="mt-3 flex items-center gap-2 rounded-xl border border-teal-200 bg-white px-3 py-2">
