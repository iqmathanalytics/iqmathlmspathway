"use client";

import {
  ArrowRight,
  Lightbulb,
  MapPin,
  Package,
  Play,
  Zap,
} from "lucide-react";
import { useLessonPractice } from "@/components/lesson/LessonPracticeContext";

function SectionLabel({
  children,
  variant,
}: {
  children: React.ReactNode;
  variant: "purple" | "green" | "blue" | "amber" | "teal";
}) {
  const styles = {
    purple: "bg-purple-100 text-purple-800",
    green: "bg-green-100 text-green-800",
    blue: "bg-blue-100 text-blue-800",
    amber: "bg-amber-100 text-amber-900",
    teal: "bg-teal-100 text-teal-800",
  };
  return (
    <span
      className={`mb-2.5 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide ${styles[variant]}`}
    >
      {children}
    </span>
  );
}

function CodeExercisePanel({
  practiceIndex,
  filename,
  children,
}: {
  practiceIndex: number;
  filename: string;
  children: React.ReactNode;
}) {
  const practice = useLessonPractice();
  const isActive = practice?.activeIndex === practiceIndex;
  const hasNext =
    practice != null && practiceIndex < practice.total - 1;

  return (
    <div
      className={`overflow-hidden rounded-xl border border-black/15 bg-white/60 transition-colors ${
        isActive ? "ring-2 ring-brand-400 ring-offset-1" : ""
      }`}
    >
      <div className="flex items-center gap-1.5 border-b border-black/10 bg-black/[0.03] px-3 py-1.5">
        <span className="h-2.5 w-2.5 rounded-full bg-[#ff5f57]" />
        <span className="h-2.5 w-2.5 rounded-full bg-[#febc2e]" />
        <span className="h-2.5 w-2.5 rounded-full bg-[#28c840]" />
        <span className="font-mono text-[11px] text-gray-500">{filename}</span>
        <div className="ml-auto flex items-center gap-1">
          <button
            type="button"
            onMouseDown={(e) => e.preventDefault()}
            onClick={() => practice?.selectPractice(practiceIndex)}
            title={isActive ? "Loaded in IDE" : "Load in IDE"}
            className={`inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[10px] font-medium transition-colors ${
              isActive
                ? "bg-brand-600 text-white"
                : "border border-brand-200 bg-white text-brand-700 hover:bg-brand-50"
            }`}
          >
            <Play className="h-2.5 w-2.5" />
            IDE
          </button>
          {isActive && hasNext && (
            <button
              type="button"
              onMouseDown={(e) => e.preventDefault()}
              onClick={() => practice?.nextPractice()}
              title="Next exercise"
              className="inline-flex items-center gap-0.5 rounded border border-gray-200 bg-white px-1.5 py-0.5 text-[10px] font-medium text-gray-600 transition hover:bg-gray-50"
            >
              Next
              <ArrowRight className="h-2.5 w-2.5" />
            </button>
          )}
        </div>
      </div>
      <pre className="overflow-x-auto bg-transparent px-4 py-3.5 font-mono text-[13.5px] leading-loose">
        {children}
      </pre>
    </div>
  );
}

const SHORTCUTS = [
  {
    symbol: "+=",
    name: "Add and assign",
    note: "Same as score = score + 5",
    example: "score += 5",
    result: "→ adds 5",
    symbolClass: "text-[#2d7a45]",
  },
  {
    symbol: "-=",
    name: "Subtract and assign",
    note: "Same as lives = lives - 1",
    example: "lives -= 1",
    result: "→ subtracts 1",
    symbolClass: "text-[#b83232]",
  },
  {
    symbol: "*=",
    name: "Multiply and assign",
    note: "Same as score = score * 2",
    example: "score *= 2",
    result: "→ doubles it",
    symbolClass: "text-[#5e3fa3]",
  },
  {
    symbol: "/=",
    name: "Divide and assign",
    note: "Same as price = price / 4",
    example: "price /= 4",
    result: "→ divides by 4",
    symbolClass: "text-[#0f6e56]",
  },
] as const;

const REF_ROWS = [
  {
    symbol: "=",
    meaning: "Assign a value",
    example: "x = 10",
    sameAs: "—",
    color: "text-[#1a5fb4]",
  },
  {
    symbol: "+=",
    meaning: "Add and assign",
    example: "x += 5",
    sameAs: "x = x + 5",
    color: "text-[#2d7a45]",
  },
  {
    symbol: "-=",
    meaning: "Subtract and assign",
    example: "x -= 3",
    sameAs: "x = x - 3",
    color: "text-[#b83232]",
  },
  {
    symbol: "*=",
    meaning: "Multiply and assign",
    example: "x *= 2",
    sameAs: "x = x * 2",
    color: "text-[#5e3fa3]",
  },
  {
    symbol: "/=",
    meaning: "Divide and assign",
    example: "x /= 4",
    sameAs: "x = x / 4",
    color: "text-[#0f6e56]",
  },
] as const;

export function AssignmentOperatorsInfographic() {
  return (
    <div className="max-w-none text-gray-900">
      <header className="mb-8 border-b border-black/10 pb-5">
        <h2 className="text-xl font-semibold tracking-tight">
          Assignment Operators
        </h2>
        <p className="mt-1 text-[13px] text-gray-500">
          Assignment operators update a variable. = stores a value; shortcuts like
          += add and assign in one step.
        </p>
      </header>

      {/* Section 1: The = operator */}
      <section className="mb-8">
        <SectionLabel variant="blue">
          <Package className="h-3 w-3" />
          The = operator
        </SectionLabel>
        <h3 className="mb-1.5 text-base font-semibold tracking-tight">
          What = really means
        </h3>
        <p className="mb-3 text-[13.5px] leading-relaxed text-gray-600">
          <code className="rounded bg-black/[0.07] px-1.5 py-0.5 font-mono text-[12.5px]">
            =
          </code>{" "}
          is not &quot;equals&quot; in math — it means{" "}
          <strong>&quot;put this value in the variable.&quot;</strong> The right
          side is calculated first, then stored into the variable on the left.
        </p>

        <CodeExercisePanel practiceIndex={0} filename="assign.py">
          <span className="text-gray-800">points = </span>
          <span className="text-[#1a5fb4]">0</span>
          {"\n"}
          <span className="text-gray-800">points = points + </span>
          <span className="text-[#1a5fb4]">10</span>
          {"\n"}
          <span className="font-semibold text-[#8b2070]">print</span>
          <span className="text-gray-800">(points) </span>
          <span className="italic text-[#5a8a5a]"># 10</span>
        </CodeExercisePanel>

        <div className="mt-2.5 flex gap-2 rounded-lg bg-black/[0.04] px-3 py-2.5 text-[13px] leading-relaxed text-gray-600">
          <Lightbulb className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" />
          <span>
            On line 2, Python calculates{" "}
            <code className="rounded bg-black/[0.07] px-1 font-mono text-[12.5px]">
              points + 10
            </code>{" "}
            first (which is 0 + 10 = 10), then stores the result back into{" "}
            <code className="rounded bg-black/[0.07] px-1 font-mono text-[12.5px]">
              points
            </code>
            . The old value is replaced.
          </span>
        </div>
      </section>

      <hr className="my-7 border-black/10" />

      {/* Section 2: Shortcut operators */}
      <section className="mb-8">
        <SectionLabel variant="green">
          <Zap className="h-3 w-3" />
          Shortcut operators
        </SectionLabel>
        <h3 className="mb-1.5 text-base font-semibold tracking-tight">
          Do math and assign in one step
        </h3>
        <p className="mb-3 text-[13.5px] leading-relaxed text-gray-600">
          These shortcuts combine a math operation with assignment. They&apos;re
          shorter to write and very common in real Python code.
        </p>

        <div className="flex flex-col gap-1.5">
          {SHORTCUTS.map((op) => (
            <div
              key={op.symbol}
              className="flex items-center gap-3 rounded-lg border border-black/10 bg-white/50 px-3.5 py-2.5"
            >
              <span
                className={`min-w-10 shrink-0 text-center font-mono text-lg font-semibold ${op.symbolClass}`}
              >
                {op.symbol}
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-[13.5px] font-semibold text-gray-900">
                  {op.name}
                </p>
                <p className="text-[12.5px] text-gray-500">{op.note}</p>
              </div>
              <p className="shrink-0 whitespace-nowrap font-mono text-[13px] text-gray-600">
                {op.example}{" "}
                <span className="text-gray-400">{op.result}</span>
              </p>
            </div>
          ))}
        </div>
      </section>

      <hr className="my-7 border-black/10" />

      {/* Section 3: Code example */}
      <section className="mb-8">
        <SectionLabel variant="green">In code</SectionLabel>
        <h3 className="mb-3 text-base font-semibold tracking-tight">
          Shortcuts in action
        </h3>

        <CodeExercisePanel practiceIndex={1} filename="shortcuts.py">
          <span className="text-gray-800">score = </span>
          <span className="text-[#1a5fb4]">50</span>
          {"\n"}
          <span className="text-gray-800">score += </span>
          <span className="text-[#1a5fb4]">5</span>
          <span className="italic text-[#5a8a5a]">
            {"    "}# same as score = score + 5
          </span>
          {"\n"}
          <span className="font-semibold text-[#8b2070]">print</span>
          <span className="text-gray-800">(score) </span>
          <span className="italic text-[#5a8a5a]"># 55</span>
          {"\n"}
          {"\n"}
          <span className="text-gray-800">score *= </span>
          <span className="text-[#1a5fb4]">2</span>
          {"\n"}
          <span className="font-semibold text-[#8b2070]">print</span>
          <span className="text-gray-800">(score) </span>
          <span className="italic text-[#5a8a5a]"># 110</span>
        </CodeExercisePanel>
      </section>

      <hr className="my-7 border-black/10" />

      {/* Section 4: Side by side */}
      <section className="mb-8">
        <SectionLabel variant="teal">Closer look</SectionLabel>
        <h3 className="mb-1.5 text-base font-semibold tracking-tight">
          Long form vs shortcut — same result
        </h3>
        <p className="mb-3.5 text-[13.5px] leading-relaxed text-gray-600">
          The shortcut operators don&apos;t do anything new — they just let you
          write less. Both versions below produce exactly the same output.
        </p>

        <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
          <div className="flex flex-col items-center gap-2 rounded-xl border border-black/15 bg-white/55 p-4 text-center">
            <span className="rounded-md bg-blue-100 px-3 py-1 font-mono text-[15px] font-semibold text-[#1a5fb4]">
              long form
            </span>
            <span className="font-mono text-sm font-semibold leading-loose text-gray-600">
              score = 50
              <br />
              score = score + 5
            </span>
            <span className="font-mono text-[22px] font-semibold text-[#1a5fb4]">
              55
            </span>
            <span className="text-xs text-gray-500">two separate steps</span>
          </div>
          <div className="flex flex-col items-center gap-2 rounded-xl border border-black/15 bg-white/55 p-4 text-center">
            <span className="rounded-md bg-green-100 px-3 py-1 font-mono text-[15px] font-semibold text-[#2d7a45]">
              shortcut
            </span>
            <span className="font-mono text-sm font-semibold leading-loose text-gray-600">
              score = 50
              <br />
              score += 5
            </span>
            <span className="font-mono text-[22px] font-semibold text-[#2d7a45]">
              55
            </span>
            <span className="text-xs text-gray-500">one step, same result</span>
          </div>
        </div>

        <div className="mt-2.5 flex gap-2 rounded-lg bg-black/[0.04] px-3 py-2.5 text-[13px] leading-relaxed text-gray-600">
          <Lightbulb className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" />
          <span>
            <code className="rounded bg-black/[0.07] px-1 font-mono text-[12.5px]">
              score += 5
            </code>{" "}
            is just a shorter way to write{" "}
            <code className="rounded bg-black/[0.07] px-1 font-mono text-[12.5px]">
              score = score + 5
            </code>
            . Python calculates the new value and stores it back — the shortcut
            saves typing and makes code easier to read.
          </span>
        </div>
      </section>

      <hr className="my-7 border-black/10" />

      {/* Section 5: Common use case */}
      <section className="mb-8">
        <SectionLabel variant="amber">Special case</SectionLabel>
        <h3 className="mb-1.5 text-base font-semibold tracking-tight">
          Where you&apos;ll see this most
        </h3>
        <p className="mb-3.5 text-[13.5px] leading-relaxed text-gray-600">
          Assignment shortcuts appear constantly in real programs — any time you
          need to update a running total, count something, or scale a value.
        </p>

        <div className="rounded-xl border border-green-300/50 bg-green-50 px-4 py-3.5 text-[13.5px] leading-relaxed text-gray-600">
          A score tracker: start at <strong className="text-[#2d7a45]">0</strong>
          , add points each round with{" "}
          <code className="rounded bg-green-100 px-1.5 py-0.5 font-mono text-[12.5px] text-[#2d7a45]">
            score += points
          </code>
          , and the variable always holds the latest total.
        </div>

        <div className="mt-2.5 flex gap-2 rounded-lg bg-black/[0.04] px-3 py-2.5 text-[13px] leading-relaxed text-gray-600">
          <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" />
          <span>
            You&apos;ll use{" "}
            <code className="rounded bg-black/[0.07] px-1 font-mono text-[12.5px]">
              +=
            </code>{" "}
            heavily once you learn loops — for example, adding up numbers in a
            list one by one:{" "}
            <code className="rounded bg-black/[0.07] px-1 font-mono text-[12.5px]">
              total += number
            </code>{" "}
            on each pass.
          </span>
        </div>
      </section>

      <hr className="my-7 border-black/10" />

      {/* Quick reference */}
      <section>
        <h3 className="mb-2.5 text-base font-semibold tracking-tight">
          Quick reference
        </h3>
        <div className="overflow-hidden rounded-xl border border-black/15 bg-white/50">
          <table className="w-full border-collapse text-[13.5px]">
            <thead>
              <tr className="border-b border-black/15 bg-black/[0.05]">
                <th className="w-14 px-3 py-2 text-center text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                  Op
                </th>
                <th className="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                  Meaning
                </th>
                <th className="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                  Example
                </th>
                <th className="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                  Same as
                </th>
              </tr>
            </thead>
            <tbody>
              {REF_ROWS.map((row) => (
                <tr
                  key={row.symbol}
                  className="border-b border-black/10 last:border-b-0"
                >
                  <td
                    className={`px-3 py-2.5 text-center font-mono text-[15px] font-semibold ${row.color}`}
                  >
                    {row.symbol}
                  </td>
                  <td className="px-3 py-2.5 text-gray-600">{row.meaning}</td>
                  <td className="px-3 py-2.5 text-gray-600">
                    <code className="rounded bg-black/[0.07] px-1 font-mono text-[12.5px] text-gray-800">
                      {row.example}
                    </code>
                  </td>
                  <td className="px-3 py-2.5 text-gray-600">
                    {row.sameAs === "—" ? (
                      "—"
                    ) : (
                      <code className="rounded bg-black/[0.07] px-1 font-mono text-[12.5px] text-gray-800">
                        {row.sameAs}
                      </code>
                    )}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </section>
    </div>
  );
}
