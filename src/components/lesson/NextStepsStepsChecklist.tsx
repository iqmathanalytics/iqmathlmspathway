...
const STEPS = [
  { id: 1, title: "Name the User Problem", snippet: "User Profile: Customer support ...", color: "emerald" },
  { id: 2, title: "Design the AI Support", snippet: "AI Task: Semantic search ...", color: "teal" },
  { id: 3, title: "Check the Result", snippet: "Test Case 1 (Happy Path) ...", color: "blue" },
] as const;
...
export function NextStepsStepsChecklist() {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      {/* ── Header ── */}
      <div className="border-b border-slate-200 bg-gradient-to-r from-cyan-50 to-sky-50 px-4 py-4">
        ...
        <p className="mt-0.5 text-[11.5px] text-slate-500">
          Copy each template cell to tick it off
        </p>
        ...
        <div className="mt-3 space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-slate-500">
              {count} of {total} templates copied
            </span>
            <span className={`text-[11px] font-bold ${allDone ? "text-cyan-700" : "text-slate-500"}`}>
              {pct}%
            </span>
          </div>
          <div className="h-2 w-full overflow-hidden rounded-full bg-slate-100">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                allDone ? "bg-cyan-500" : "bg-cyan-400"
              }`}
              style={{ width: `${pct}%` }}
            />
          </div>
        ...
        {allDone && (
          <div className="mt-3 flex items-center gap-2 rounded-xl border border-cyan-200 bg-white px-3 py-2">
            <CheckCircle2 className="h-4 w-4 text-cyan-500" />
            <p className="text-[12px] font-semibold text-cyan-700">
              All templates copied — great work!
            </p>
          </div>
        )}
      </div>
