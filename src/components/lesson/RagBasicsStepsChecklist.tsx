...
const STEPS = [
  ...
  },
  {
    ...
    snippet: "def simple_retrieve(query, docs): ...",
    color: "blue",
  },
  {
    id: 4,
    title: "Initialize Groq Client",
    snippet: "client = Groq()",
    color: "violet",
  },
  {
    id: 5,
    title: "Run Grounded Generation",
...
export function RagBasicsStepsChecklist() {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      {/* ── Header ── */}
      <div className="border-b border-slate-200 bg-gradient-to-r from-indigo-50 to-violet-50 px-4 py-4">
        ...
        <div className="mt-3 space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-[11px] font-bold ${allDone ? "text-indigo-600" : "text-slate-500"}`}>
              {pct}%
            </span>
          </div>
          <div className="h-2 w-full overflow-hidden rounded-full bg-slate-100">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                allDone ? "bg-indigo-500" : "bg-indigo-400"
              }`}
              style={{ width: `${pct}%` }}
            />
          </div>
        ...
        {allDone && (
          <div className="mt-3 flex items-center gap-2 rounded-xl border border-indigo-200 bg-white px-3 py-2">
